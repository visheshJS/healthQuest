      ...
      <div className="container mx-auto px-4 py-3">
        <div className="flex items-center justify-between">
          <Link to="/" className="flex items-center space-x-2">
            <Heart className="w-6 h-6 sm:w-7 sm:h-7 text-green-300" />
          ...
          <div className="flex items-center space-x-2 sm:space-x-3">
            <div className="flex items-center space-x-1">
              {[...Array(5)].map((_, i) => (
                <div
                  key={i}
                  className={`w-3 h-3 sm:w-5 sm:h-5 rounded-full transition-all duration-300 transform ${
                    i < health ? 'bg-red-500 scale-100' : 'bg-red-900/40 scale-90'
                  }`}
                />
              ))}
            </div>
            <div className="flex items-center space-x-1 sm:space-x-2 bg-green-800/80 rounded-lg px-2 sm:px-4 py-1 sm:py-2">
              <Trophy className="w-4 h-4 sm:w-5 sm:h-5 text-yellow-400" />
              <span className="text-white text-sm sm:text-base font-bold">{score}</span>
            </div>
            <Link to="/dashboard">
              <div className="flex items-center space-x-1 sm:space-x-2 hover:bg-green-800/80 rounded-lg px-2 sm:px-4 py-1 sm:py-2 transition-all duration-300">
    ...
    return (
      <div className="relative flex flex-col min-h-screen bg-gradient-to-b from-green-900 via-green-800 to-emerald-900 overflow-hidden">
        ...
        <div className="flex items-center justify-center flex-grow p-4">
          <div className="text-lg sm:text-2xl text-green-300 flex flex-col sm:flex-row items-center">
            <Heart className="w-6 h-6 sm:w-8 sm:h-8 text-green-300 animate-pulse mb-3 sm:mb-0 sm:mr-3" />
            <span className="animate-pulse text-center sm:text-left">Loading anatomy questions...</span>
          </div>
        </div>
      </div>
    );
      ...
      <div className="relative flex flex-col min-h-screen bg-gradient-to-b from-green-900 via-green-800 to-emerald-900 overflow-hidden">
        ...
        <div className="flex flex-col items-center justify-center flex-grow p-4 sm:p-6 pt-20 sm:pt-24">
          <div className="game-card p-4 sm:p-8 shadow-2xl max-w-lg w-full text-center transform hover:scale-105 transition-all duration-300">
            ...
            <h2 className="text-2xl sm:text-4xl font-russo mb-3 sm:mb-6 text-green-300">Quiz Complete!</h2>
            <div className="space-y-2 sm:space-y-4 mb-4 sm:mb-8">
              <p className="text-lg sm:text-2xl text-white">Final Score: <span className="text-green-300">{score}/{currentQuestionIndex + 1}</span></p>
              <div className="w-full bg-green-900/50 rounded-full h-3 sm:h-4">
                <div 
                  className="bg-gradient-to-r from-green-400 to-emerald-300 h-3 sm:h-4 rounded-full"
                  style={{ width: `${percentage}%` }}
                ></div>
              </div>
              <p className="text-base sm:text-xl text-white">Accuracy: <span className="text-green-300">{percentage}%</span></p>
              <p className="text-base sm:text-xl text-white">Health Remaining: <span className="text-red-400">{health}</span></p>
            </div>
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 justify-center">
              <button 
                onClick={() => window.location.reload()}
                className="game-button py-2 px-4 text-sm sm:text-base"
              >
                Play Again
              </button>
              <button 
      ...
      <div className="flex flex-col items-center justify-center flex-grow p-4 sm:p-6 pt-20 sm:pt-24">
        ...
        <div className="w-full max-w-3xl mb-4 sm:mb-8">
          <div className="flex justify-between items-center mb-2">
            <span className="text-xs sm:text-sm text-green-300">Question {currentQuestionIndex + 1} of {Math.min(questions.length, 10)}</span>
            <span className="text-xs sm:text-sm text-green-300">Score: {score}</span>
          </div>
          <div className="bg-green-800/50 rounded-full h-2 sm:h-3 shadow-lg p-0.5">
            <div
              className="bg-gradient-to-r from-green-400 to-emerald-300 h-1 sm:h-2 rounded-full transition-all duration-500"
        ...
        {/* Question Card */}
        <div className="w-full max-w-3xl game-card p-4 sm:p-8 mb-4 sm:mb-8 transform hover:scale-[1.01] transition-all duration-300">
          <h2 className="text-xl sm:text-2xl font-russo mb-4 sm:mb-8 text-green-300">{currentQuestion?.question}</h2>
          <div className="grid gap-3 sm:gap-4">
            {currentQuestion?.options.map((option, index) => (
              <button
                key={index}
                onClick={() => handleOptionSelect(option)}
                className={`p-3 sm:p-4 text-left rounded-xl text-base sm:text-lg transition-all duration-300 transform hover:scale-[1.02] border ${
                  selectedOption === null
          ...
          {selectedOption && (
            <div className={`mt-4 sm:mt-6 p-3 sm:p-4 rounded-lg text-sm sm:text-base ${isCorrect ? 'bg-green-600/20 border border-green-500/30' : 'bg-red-600/20 border border-red-500/30'}`}>
        ...
        {!selectedOption && (
          <button
            onClick={handleSkip}
            className="game-button-small py-2 sm:py-3 px-4 sm:px-6 text-sm sm:text-base"
          >
            Skip Question
          </button>
        )}
