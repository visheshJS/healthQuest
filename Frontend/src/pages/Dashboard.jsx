    ...
    <div className="min-h-screen bg-gradient-to-b from-green-900 via-green-800 to-emerald-900 text-white relative overflow-hidden">
      ...
      <main className="container mx-auto px-4 py-4 sm:py-8 relative z-10">
        ...
        <div className="mb-6 sm:mb-10">
          ...
          <p className="text-sm sm:text-base text-green-100/70">Your medical adventure continues. What would you like to play today?</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 mb-6 sm:mb-10">
          <div className="game-card p-3 sm:p-4">
            <div className="flex justify-between items-start mb-2 sm:mb-3">
              <div>
                <p className="text-green-300/70 text-xs sm:text-sm">Current Level</p>
                <h3 className="text-xl sm:text-2xl font-bold text-white">{user?.level || 1}</h3>
              </div>
          ...
          <div className="game-card p-3 sm:p-4">
            <div className="flex justify-between items-start mb-2 sm:mb-3">
              <div>
                <p className="text-green-300/70 text-xs sm:text-sm">Learning Streak</p>
                <h3 className="text-xl sm:text-2xl font-bold text-white">{user?.streak || 0} days</h3>
              </div>
              <div className="p-1.5 sm:p-2 bg-green-500/20 rounded-lg">
                <Award className="text-green-300" size={20} />
              </div>
            </div>
            <p className="text-xs sm:text-sm text-green-100/70 mt-2">Keep your streak going! Log in tomorrow to continue learning.</p>
          </div>
          
          <div className="game-card p-3 sm:p-4 sm:col-span-2 md:col-span-1">
            <div className="flex justify-between items-start mb-2 sm:mb-3">
              <div>
                <p className="text-green-300/70 text-xs sm:text-sm">Quizzes Completed</p>
                <h3 className="text-xl sm:text-2xl font-bold text-white">{user?.completedQuizzes || 0}</h3>
              </div>
              <div className="p-1.5 sm:p-2 bg-green-500/20 rounded-lg">
                <Brain className="text-green-300" size={20} />
              </div>
            </div>
            <p className="text-xs sm:text-sm text-green-100/70 mt-2">You've completed {user?.completedQuizzes || 0} quizzes so far.</p>
          </div>
        </div>
        
        {/* Game Options */}
        <h2 className="text-xl sm:text-3xl font-russo mb-4 sm:mb-6 text-green-300">Game Modes</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-10">
          <div className="game-card p-4 sm:p-6 cursor-pointer transform hover:scale-105 transition-all duration-300" onClick={() => handlePlayGame('diagnosis')}>
            <div className="flex gap-3 sm:gap-4 items-start">
              ...
              <div className="flex-1">
                <h3 className="text-lg sm:text-2xl font-russo mb-1 sm:mb-2 text-green-300">Diagnosis Detective</h3>
                <p className="text-xs sm:text-sm text-green-100/70 mb-2 sm:mb-3">Solve medical mysteries by analyzing symptoms and patient histories</p>
                <div className="flex items-center justify-between">
                  <div className="flex">
                    {[...Array(5)].map((_, i) => (
                      <Star key={`diagnosis-star-${i}`} className="w-3 h-3 sm:w-4 sm:h-4 text-green-400/80 mr-1" fill={i < 4 ? "currentColor" : "none"} />
                    ))}
                  </div>
          ...
          <div className="game-card p-4 sm:p-6 cursor-pointer transform hover:scale-105 transition-all duration-300" onClick={() => handlePlayGame('anatomy')}>
            <div className="flex gap-3 sm:gap-4 items-start">
              ...
              <div className="flex-1">
                <h3 className="text-lg sm:text-2xl font-russo mb-1 sm:mb-2 text-green-300">Anatomy Quiz</h3>
                <p className="text-xs sm:text-sm text-green-100/70 mb-2 sm:mb-3">Test your knowledge of human anatomy through interactive challenges</p>
                <div className="flex items-center justify-between">
                  <div className="flex">
                    {[...Array(5)].map((_, i) => (
                      <Star key={`anatomy-star-${i}`} className="w-3 h-3 sm:w-4 sm:h-4 text-green-400/80 mr-1" fill={i < 3 ? "currentColor" : "none"} />
                    ))}
                  </div>
                  <span className="text-xs bg-green-500/20 text-green-300 px-2 py-0.5 rounded">Medium</span>
                </div>
              </div>
            </div>
            <button className="mt-3 sm:mt-4 w-full game-button-small py-1.5 sm:py-2 text-sm">
          ...
          <div className="game-card p-4 sm:p-6 cursor-pointer transform hover:scale-105 transition-all duration-300" onClick={() => handlePlayGame('trivia')}>
            <div className="flex gap-3 sm:gap-4 items-start">
              ...
              <div className="flex-1">
                <h3 className="text-lg sm:text-2xl font-russo mb-1 sm:mb-2 text-green-300">Medical Trivia</h3>
                <p className="text-xs sm:text-sm text-green-100/70 mb-2 sm:mb-3">Test your medical knowledge with challenging trivia questions</p>
                <div className="flex items-center justify-between">
                  <div className="flex">
                    {[...Array(5)].map((_, i) => (
                      <Star key={`trivia-star-${i}`} className="w-3 h-3 sm:w-4 sm:h-4 text-green-400/80 mr-1" fill={i < 3 ? "currentColor" : "none"} />
                    ))}
                  </div>
                  <span className="text-xs bg-green-500/20 text-green-300 px-2 py-0.5 rounded">Medium</span>
                </div>
            ...
            <div className="space-y-3 sm:space-y-4">
              {user?.recentGames && user.recentGames.length > 0 ? (
                user.recentGames.map((game, index) => (
                  <div key={`${game.id}-${index}`} className="border-b border-green-700/30 pb-3 sm:pb-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center">
                        ...
                        <div>
                          <h3 className="text-sm sm:text-base font-medium text-white">{game.type}</h3>
                          <p className="text-xs sm:text-sm text-green-200/70">Score: {game.score}% · {game.xp} XP earned</p>
                        </div>
                      </div>
                      <span className="text-xs sm:text-sm text-green-200/50">{formatTimeAgo(game.timestamp)}</span>
                    </div>
                  </div>
                ))
              ) : (
                <div className="text-center py-4 sm:py-6 text-green-200/70">
                  <p className="text-sm">No recent activity yet. Start playing to see your progress!</p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-3 sm:space-y-4">
              {user?.achievements && user.achievements.map((achievement, index) => (
                <div key={`${achievement.id}-${index}`} className={`flex items-center ${!achievement.achieved ? 'opacity-50' : ''}`}>
                  <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full ${achievement.achieved ? 'bg-green-500/20' : 'bg-green-800/60'} flex items-center justify-center mr-3 sm:mr-4`}>
                    <Trophy className={achievement.achieved ? 'text-green-300' : 'text-green-500/50'} size={20} />
                  </div>
                  <div>
                    <h3 className={`text-sm sm:text-base font-medium ${achievement.achieved ? 'text-white' : 'text-green-300/50'}`}>
                      {achievement.name}
                    </h3>
                    <p className={`text-xs sm:text-sm ${achievement.achieved ? 'text-green-200/70' : 'text-green-200/50'}`}>
