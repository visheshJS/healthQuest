  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join-room', async (data) => {
      try {
        ...
        if (room.host.id === userId) {
          socket.emit('error-message', { message: 'You cannot join your own room!' });
          return;
        }
        
        // Check if the same socket is trying to join (prevents joining from same browser tab)
        if (room.host.socketId === socket.id) {
          socket.emit('error-message', { message: 'Cannot join from the same connection!' });
          return;
        }
        ...
        setTimeout(() => {
          console.log(`Starting game in room ${roomCode}. Host: ${room.host.username} (${room.host.id}), Guest: ${room.guest.username} (${room.guest.id})`);
          ...
          // Also send initial scores to ensure both players start with correct values
          
          // Initial score update for host
          const hostScoreUpdate = {
            playerScore: 0,
            opponentScore: 0,
            playerName: room.host.username,
          ...
          // Initial score update for guest
          const guestScoreUpdate = {
            playerScore: 0,
            opponentScore: 0,
            playerName: room.guest.username,
          ...
          console.log(`Sending initial scores to players in room ${roomCode}`);
          console.log(`Host (${room.host.username}): `, JSON.stringify(hostScoreUpdate));
          console.log(`Guest (${room.guest.username}): `, JSON.stringify(guestScoreUpdate));
          
          // Send custom score update to each player
          io.to(room.host.socketId).emit('score-update', hostScoreUpdate);
          io.to(room.guest.socketId).emit('score-update', guestScoreUpdate);
    ...
    socket.on('player-answer', async (data) => {
      try {
        ...
        console.log(`RECEIVED ANSWER from socket ${socket.id}: Room ${roomCode}, Q${questionIndex}, A${answerIndex}, Correct: ${isCorrect}`);
        
        if (!gameRooms.has(roomCode)) {
          console.log(`Room ${roomCode} not found in gameRooms map. Available rooms: ${Array.from(gameRooms.keys()).join(', ')}`);
          return;
        }
        
        const room = gameRooms.get(roomCode);
        console.log(`Room found. Host: ${room.host.username} (${room.host.socketId}), Guest: ${room.guest?.username} (${room.guest?.socketId})`);
        ...
        if (!player) {
          console.error(`Player not found for socket ${socket.id} in room ${roomCode}`);
          return;
        }
        ...
        if (isCorrect) {
          player.score += 10;
          console.log(`UPDATING SCORE: Player ${player.username} (${player.id}) score updated to ${player.score}`);
        }
        
        // Create separate score update objects for each player
        // The key is to swap the playerScore and opponentScore fields based on who is receiving the message
        
        // Score update for the player who answered
        const playerScoreUpdate = {
          ...
          hostId: room.host.id,
          guestId: room.guest ? room.guest.id : null,
          hostName: room.host.username,
          guestName: room.guest ? room.guest.username : null
        };
        
        // Score update for the opponent
        const opponentScoreUpdate = {
          playerScore: opponent ? opponent.score : 0,
        ...
        console.log("=== SCORE UPDATES BEING SENT ===");
        console.log(`Player (${player.username}): `, JSON.stringify(playerScoreUpdate));
        console.log(`Opponent (${opponent?.username}): `, JSON.stringify(opponentScoreUpdate));
        
        // Send scores directly to player who answered
        io.to(player.socketId).emit('score-update', playerScoreUpdate);
        
        // Send scores directly to opponent
        if (opponent && opponent.socketId) {
          io.to(opponent.socketId).emit('score-update', opponentScoreUpdate);
        }
        
        // Also send individual answer notification to opponent
        if (opponent && opponent.socketId) {
          console.log(`Sending answer details to opponent ${opponent.username} (${opponent.socketId})`);
          ...
          console.log(`No opponent found or opponent has no socketId: ${JSON.stringify(opponent)}`);
        }
      } catch (error) {
